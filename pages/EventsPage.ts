...
export class EventsPage extends BasePage {
    ...
    readonly eventMoreButton: Locator;
    ...
    constructor(page: Page) {
        ...
        this.emptyStateMessage = page.locator('.end-page-txt');
        this.eventMoreButton = page.getByRole('button', { name: 'More' });
        this.clearSearchButton = page.getByAltText('cancel search');
     }
    ...
    async checkAllEventCardsLocationContains(location: string) {
        await step(`Verify all event cards have location containing: ${location}`, async () => {
            await this.eventCards.first().waitFor({ timeout: 5000 });
            const count = await this.getEventItemsCount();
            await expect(count).toBeGreaterThan(0);
            for (let i = 0; i < count; i++) {
    ...
    async clickMoreButtonOfEventByTitle(title: string) {
        const count = await this.getEventItemsCount();
        await expect(count).toBeGreaterThan(0);
        for (let i = 0; i < count; i++) {
            const eventCard = this.getEventCardByIndex(i);
            ...
            if (cardTitle.includes(title)) {
                const moreButton = eventCard.getMoreButton();
                await moreButton.click();
                return new EventItemPage(this.page);
            }
        }
    }
    ...
    async applyEventTimeFilter(eventTime: EventTimeFilter) {
        await step(`Apply event time filter: ${eventTime}`, async () => {
            await expect(this.page.getByRole('option', { name: eventTime })).toBeVisible();
    ...
    async applyLocationFilter(city: string) {
        await step(`Apply location filter: ${city}`, async () => {
            await this.page.locator('mat-option.ng-star-inserted span.mdc-list-item__primary-text')
    ...
    async checkEmptyStateMessage() {
        await step('Verify empty state message', async () => {
            ...
            const actualMessage = await this.emptyStateMessage.innerText();
            await expect(actualMessage).toBe("We didn't find any results matching to this search");
        });
    }
